...
export default async function CarDetails({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader/><main><nav className="container catalog-breadcrumb" aria-label="Breadcrumb"><Link href="/">Home</Link><span aria-hidden="true">/</span><Link href="/cars">Used cars</Link><span aria-hidden="true">/</span><strong aria-current="page">{car.brand} {car.model}</strong></nav><div className="container details-top"><Link href="/cars" className="back-link"><ArrowLeft size={17}/> Back to cars</Link><div className="details-heading"><div><div className="eyebrow dark"><span className="eyebrow-line"/> AVAILABLE CAR</div><h1>{car.brand} {car.model}</h1><p><MapPin size={16}/>{car.location}{typeof car.viewCount === 'number' && car.viewCount > 0 && <span className="detail-view-count"><Eye size={14}/>{car.viewCount} {car.viewCount === 1 ? 'view' : 'views'}</span>}</p></div><div className="detail-price"><small>Asking price</small><strong>{money(car.price)}</strong></div></div>
