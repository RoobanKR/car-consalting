...
import { BUSINESS, HOME_FAQ, faqJsonLd } from '@/lib/seo';
...
export default async function Home() {
  ...
  return <><SiteHeader/><main className="market-home">
    <section className={`market-hero${heroVideo ? ' market-hero-video' : ''}`} style={heroStyle}>{heroVideo && <><video className="market-hero-media" autoPlay muted loop playsInline preload="auto" poster={heroVideo.posterUrl || undefined} src={heroVideo.url}/><span className="market-hero-scrim" aria-hidden="true"/></>}<div className="container market-hero-inner"><div className="market-hero-copy"><span className="market-hero-kicker"><Sparkles size={15}/> USED CARS &amp; CAR CONSULTING IN MUSIRI</span><h1>Used cars in Musiri<br/><em>you&apos;ll love to drive.</em></h1><p>KangaCars helps you buy the right second-hand car in Musiri, Thottiyam, Thuraiyur and Trichy. Compare the details and get honest car consulting before you decide.</p><form action="/cars" className="market-hero-search"><Search size={21}/><input name="search" aria-label="Search cars" placeholder="Search used cars by brand, model or city"/><button type="submit">Search cars <ArrowRight size={18}/></button></form><div className="market-hero-caption"><span>Popular searches</span><Link href="/cars?body=SUV">SUV <ChevronRight size={13}/></Link><Link href="/cars?body=Sedan">Sedan <ChevronRight size={13}/></Link><Link href="/cars?body=Hatchback">Hatchback <ChevronRight size={13}/></Link></div></div></div></section>
    ...
    <section className="market-local" aria-labelledby="local-heading"><div className="container market-local-grid"><div><span className="market-overline">KANGACARS, MUSIRI</span><h2 id="local-heading">Car sale &amp; car consulting in Musiri</h2><p>KangaCars is a used car sales and car consulting service based in {BUSINESS.street}, Musiri, in {BUSINESS.district} district, Tamil Nadu. Whether you want to buy a used car in Musiri or sell your own, we help you find a fair price, check the car&apos;s condition and papers, and complete the deal without stress.</p><p>Every listing shows the asking price, kilometres, fuel type and photos up front. Found one you like? Send an enquiry or message us on WhatsApp and we will arrange a viewing in Musiri.</p><h3>Areas we serve</h3><ul className="market-local-areas">{BUSINESS.areaServed.map(area => <li key={area}>{area}</li>)}</ul><div className="market-local-links"><Link href="/cars">Used cars for sale in Musiri <ArrowRight size={16}/></Link><Link href="/contact">Talk to a car consultant <ArrowRight size={16}/></Link></div></div><div className="market-local-faq"><h3>Frequently asked questions</h3>{HOME_FAQ.map(item => <details key={item.q}><summary>{item.q}</summary><p>{item.a}</p></details>)}</div></div></section>
