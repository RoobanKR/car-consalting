...
import { ArrowLeft, ArrowRight, ArrowUpDown, ArrowUp, ArrowDown, AlertTriangle, CalendarDays, CarFront, ChevronRight, Check, Eye, Fuel, Gauge, Home, Image as ImageIcon, Inbox, KeyRound, LayoutDashboard, LogOut, Mail, MapPin, Phone, Plus, RotateCcw, Save, Search, Settings2, ShieldCheck, Sparkles, Trash2, Upload, User as UserIcon, Users, Video, X } from 'lucide-react';
import { api, Car, Enquiry, HeroMedia, User, money } from '@/lib/api';
import FeaturePicker from '@/components/FeaturePicker';
import VehicleFields from '@/components/VehicleFields';

type Tab = 'dashboard' | 'cars' | 'enquiries' | 'appearance' | 'users';
...
const emptyCar = (): CarForm => ({ brand: '', model: '', year: new Date().getFullYear(), price: 0, fuelType: 'Petrol', transmission: 'Manual', kmDriven: 0, bodyType: '', location: '', description: '', features: [], images: [], status: 'active', postedBy: { name: 'Carwise Admin', email: '', phone: '' } });
...
export default function AdminWorkspace({ superadmin = false }: { superadmin?: boolean }) {
  ...
  const load = useCallback(async (auth: string) => {
    try {
      const [carData, enquiryData, heroData, me] = await Promise.all([api<Car[]>('/admin/cars', {}, auth), api<Enquiry[]>('/admin/enquiries', {}, auth), api<HeroMedia[]>('/admin/hero', {}, auth), api<User>('/admin/me', {}, auth)]);
      setCars(carData); setEnquiries(enquiryData); setHeroMedia(heroData); setSignedInAs(me); setError('');
  ...
  function logout() { sessionStorage.removeItem('carwise-admin-token'); setToken(''); setCars([]); setEnquiries([]); setHeroMedia([]); setUsers([]); setSignedInAs(null); }
  ...
  if (!token) return <div className="admin-login"><div className="admin-login-side"><Link href="/" className="brand"><span className="brand-mark">C<span>.</span></span><span>carwise<span className="brand-dot">.</span></span></Link><div><span className="eyebrow"><span className="eyebrow-line"/> {superadmin ? 'SUPERADMIN WORKSPACE' : 'ADMIN WORKSPACE'}</span><h1>Manage the cars.<br/><em>Follow every lead.</em></h1><p>Your inventory and enquiries, together in one place.</p></div><small>CARWISE / {superadmin ? 'SUPERADMIN' : 'ADMIN'}</small></div><div className="admin-login-form"><form onSubmit={login}><div className="admin-icon"><CarFront/></div><h2>Welcome back</h2><p>Sign in to your admin dashboard.</p><label>Email address<input value={email} onChange={e => setEmail(e.target.value)} type="email" required placeholder="admin@example.com"/></label><label>Password<input value={password} onChange={e => setPassword(e.target.value)} type="password" required placeholder="Your password"/></label>{loginError && <p className="form-error">{loginError}</p>}<button disabled={busy} className="button button-primary full">{busy ? 'Signing in...' : 'Sign in'} <ArrowRight size={18}/></button><Link href="/" className="back-link"><ArrowLeft size={16}/> Back to website</Link></form></div></div>;
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Link href="/" className="brand"><span className="brand-mark">C<span>.</span></span><span>carwise<span className="brand-dot">.</span></span></Link>{canManageUsers && <span className="sidebar-role"><ShieldCheck size={13}/> SUPERADMIN</span>}<span className="sidebar-label">WORKSPACE</span><nav><button className={tab === 'dashboard' ? 'active' : ''} onClick={() => { setTab('dashboard'); setEditing(null); }}><LayoutDashboard size={19}/> Overview</button><button className={tab === 'cars' ? 'active' : ''} onClick={() => { setTab('cars'); setEditing(null); }}><CarFront size={19}/> Cars <span>{cars.length}</span></button><button className={tab === 'enquiries' ? 'active' : ''} onClick={() => { setTab('enquiries'); setEditing(null); }}><Inbox size={19}/> Enquiries <span>{newEnquiries}</span></button><button className={tab === 'appearance' ? 'active' : ''} onClick={() => { setTab('appearance'); setEditing(null); }}><Sparkles size={19}/> Header media</button>{canManageUsers && <button className={tab === 'users' ? 'active' : ''} onClick={() => { setTab('users'); setEditing(null); setEditingUser(null); }}><Users size={19}/> Users <span>{users.length}</span></button>}</nav><div className="sidebar-bottom"><Link href="/"><ArrowLeft size={17}/> View website</Link><button onClick={logout}><LogOut size={17}/> Sign out</button></div></aside>
    <main className="admin-main"><header className="admin-topbar"><span>{superadmin ? 'CARWISE SUPERADMIN' : 'CARWISE ADMIN'} <ChevronRight size={14}/> {editing ? (editing._id ? 'Edit car' : 'Add car') : tab === 'appearance' ? 'Header media' : tab}</span><div className="admin-topbar-right"><span className="admin-avatar">A</span><button type="button" className="admin-topbar-signout" onClick={logout} aria-label="Sign out"><LogOut size={17}/></button></div></header><div className="admin-content">
  ...
  <td><div className="table-poster"><button type="button" className="table-poster-name" onClick={event => { event.stopPropagation(); setViewingPoster(car); }} title="View poster contact"><UserIcon size={12}/>{car.postedBy?.name || 'Carwise Admin'}</button><small>{formatDate(car.createdAt)}</small></div></td>
