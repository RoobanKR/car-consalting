...
type Report = {
  ...
  enquiries: {
    selected: { total: number; new: number; contacted: number; closed: number };
    quick: { week: number; month: number; year: number; allTime: number };
    monthly: { month: number; label: string; count: number }[];
    topCars: { carId: string; car: string; slug: string; count: number; lastAt?: string }[];
    recent: { _id: string; name: string; email: string; phone: string; city: string; message: string; status: string; car: string; slug: string; createdAt?: string }[];
  };
...
const initials = (person?: User | null) => (person ? `${person.firstName.charAt(0)}${person.lastName.charAt(0)}`.toUpperCase() : '') || 'A';
...
export default function AdminWorkspace({ superadmin = false }: { superadmin?: boolean }) {
  ...
  // The account card is a popover, so it closes the way one is expected to:
  // a click anywhere else, or Escape.
  useEffect(() => {
    if (!accountOpen) return;
    const clickAway = (event: MouseEvent) => { if (!accountRef.current?.contains(event.target as Node)) setAccountOpen(false); };
  ...
  /** A new listing is credited to whoever is signed in; an existing one keeps the
   *  person already on it. Either way the fields stay editable, since an admin may
   *  be listing a car on a colleague's behalf. */
  function newCarForm(): CarForm {
    const blank = emptyCar();
    return { ...blank, postedBy: {
      name: fullName(signedInAs) || blank.postedBy?.name || '',
      email: signedInAs?.email || '',
      phone: signedInAs?.phone || ''
    } };
  }

  function openCar(car?: Car) { const fresh = newCarForm(); setEditing(car || ({ ...fresh, _id: '' } as Car)); setForm(car ? { ...car } : fresh); setError(''); setTab('cars'); }
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><Link href="/" className="brand"><span className="brand-logo"><img src="/kangacars-logo.png" alt="KangaCars" width={620} height={208}/></span></Link>{canManageUsers && <span className="sidebar-role"><ShieldCheck size={13}/> SUPERADMIN</span>}<span className="sidebar-label">WORKSPACE</span><nav><button className={tab === 'dashboard' ? 'active' : ''} onClick={() => { setTab('dashboard'); setEditing(null); }}><LayoutDashboard size={19}/> Overview</button><button className={tab === 'cars' ? 'active' : ''} onClick={() => { setTab('cars'); setEditing(null); }}><CarFront size={19}/> Cars <span>{cars.length}</span></button><button className={tab === 'enquiries' ? 'active' : ''} onClick={() => { setTab('enquiries'); setEditing(null); }}><Inbox size={19}/> Enquiries <span>{newEnquiries}</span></button><button className={tab === 'appearance' ? 'active' : ''} onClick={() => { setTab('appearance'); setEditing(null); }}><Sparkles size={19}/> Header media</button><button className={tab === 'reports' ? 'active' : ''} onClick={() => { setTab('reports'); setEditing(null); }}><BarChart3 size={19}/> Reports</button><button className={tab === 'feedback' ? 'active' : ''} onClick={() => { setTab('feedback'); setEditing(null); setEditingFeedback(null); }}><MessageSquareText size={19}/> Feedback <span>{feedback.length}</span></button>{canManageUsers && <button className={tab === 'users' ? 'active' : ''} onClick={() => { setTab('users'); setEditing(null); setEditingUser(null); }}><Users size={19}/> Users <span>{users.length}</span></button>}</nav><div className="sidebar-bottom"><Link href="/"><ArrowLeft size={17}/> View website</Link><button onClick={logout}><LogOut size={17}/> Sign out</button></div></aside>
    <main className="admin-main"><header className="admin-topbar"><span>{superadmin ? 'KANGACARS SUPERADMIN' : 'KANGACARS ADMIN'} <ChevronRight size={14}/> {editing ? (editing._id ? 'Edit car' : 'Add car') : tab === 'appearance' ? 'Header media' : tab}</span><div className="admin-topbar-right"><div className="admin-account" ref={accountRef}><button type="button" className="admin-avatar" onClick={() => setAccountOpen(open => !open)} aria-haspopup="dialog" aria-expanded={accountOpen} aria-label="Your account">{initials(signedInAs)}</button>{accountOpen && <div className="admin-account-card" role="dialog" aria-label="Your account"><div className="admin-account-head"><span className="admin-account-avatar">{initials(signedInAs)}</span><div><strong>{fullName(signedInAs) || 'KangaCars Admin'}</strong><small>{signedInAs?.role === 'superadmin' ? 'Superadmin' : 'Admin'}</small></div></div><div className="admin-account-body"><div className="admin-poster-row"><Mail size={15}/><div><small>Email</small>{signedInAs?.email ? <a href={`mailto:${signedInAs.email}`}>{signedInAs.email}</a> : <strong className="admin-poster-empty">Not provided</strong>}</div></div><div className="admin-poster-row"><Phone size={15}/><div><small>Phone</small>{signedInAs?.phone ? <a href={`tel:${signedInAs.phone}`}>{signedInAs.phone}</a> : <strong className="admin-poster-empty">Not provided</strong>}</div></div>{signedInAs?.address && <div className="admin-poster-row"><MapPin size={15}/><div><small>Address</small><strong>{signedInAs.address}</strong></div></div>}</div><button type="button" className="admin-account-signout" onClick={logout}><LogOut size={16}/> Sign out</button></div>}</div><button type="button" className="admin-topbar-signout" onClick={logout} aria-label="Sign out"><LogOut size={17}/></button></div></header><div className="admin-content">
      {error && <div className="admin-alert" role="alert">{error}<button onClick={() => setError('')}><X size={16}/></button></div>}
      {tab === 'dashboard' && !editing && <><div className="admin-page-head"><div><span className="admin-kicker">YOUR WORKSPACE</span><h1>{signedInAs ? `Welcome back, ${signedInAs.firstName}` : 'Overview'}<span className="accent-dot">.</span></h1><p>Keep your inventory current and every enquiry moving.</p></div><button className="button button-primary" onClick={() => openCar()}><Plus size={18}/> Add car</button></div><div className="admin-stats"><div><span>Active cars</span><strong>{cars.filter(car => car.status === 'active').length}</strong><CarFront/></div><div><span>Sold cars</span><strong>{cars.filter(car => car.status === 'sold').length}</strong><CarFront/></div><div><span>New enquiries</span><strong>{newEnquiries}</strong><Inbox/></div><div><span>Total car views</span><strong>{cars.reduce((sum, car) => sum + (car.viewCount || 0), 0).toLocaleString('en-IN')}</strong><Eye/></div></div><div className="admin-two-col"><section className="admin-panel"><div className="panel-title"><h2>Most visited cars</h2><button onClick={() => setTab('cars')}>View all <ArrowRight size={15}/></button></div>{cars.length ? [...cars].sort((a, b) => (b.viewCount || 0) - (a.viewCount || 0)).slice(0, 5).map(car => <div className="mini-row" key={car._id}><span className="mini-car">{car.images?.[0]?.url ? <img src={car.images[0].url} alt=""/> : <CarFront size={20}/>}</span><div><strong>{car.brand} {car.model}</strong><small>{car.year} · {car.location}</small></div><span className="mini-views"><Eye size={13}/>{(car.viewCount || 0).toLocaleString('en-IN')}</span></div>) : <p className="panel-empty">No cars yet. Add your first listing.</p>}</section><section className="admin-panel"><div className="panel-title"><h2>Recent enquiries</h2><button onClick={() => setTab('enquiries')}>View all <ArrowRight size={15}/></button></div>{enquiries.length ? enquiries.slice(0, 5).map(enquiry => <div className="mini-row" key={enquiry._id}><span className="initial">{enquiry.name.charAt(0)}</span><div><strong>{enquiry.name}</strong><small>{enquiry.carId ? `${enquiry.carId.brand} ${enquiry.carId.model}` : 'Car removed'}</small></div><span className={`status status-${enquiry.status}`}>{enquiry.status}</span></div>) : <p className="panel-empty">No enquiries yet.</p>}</section></div></>}
  ...
  <td><div className="row-actions" onClick={event => event.stopPropagation()}><button onClick={() => openCar(car)}>Edit</button><button className="sale-action" onClick={() => openSale(car)}>{car.status === 'sold' ? 'Sale details' : 'Mark sold'}</button><button onClick={() => setDeleting(car)} aria-label={`Delete ${car.brand} ${car.model}`}><Trash2 size={16}/></button></div></td>
      ...
      {tab === 'cars' && editing && <><div className="admin-page-head"><div><button className="back-link" onClick={() => setEditing(null)}><ArrowLeft size={17}/> Back to cars</button><h1>{editing._id ? 'Edit car' : 'Add a car'}<span className="accent-dot">.</span></h1><p>Give buyers the details they need to enquire with confidence.</p></div></div><form className="admin-car-form" onSubmit={saveCar}><section className="admin-panel"><h2>Basic details</h2><div className="admin-form-grid"><VehicleFields key={editing._id || "new"} value={{ brand: form.brand, model: form.model, bodyType: form.bodyType }} inventory={cars} onChange={vehicle => setForm(current => ({ ...current, ...vehicle }))}/><label>Year<input type="number" min="1980" max={new Date().getFullYear() + 1} required value={form.year} onChange={event => setForm({ ...form, year: Number(event.target.value) })}/></label><label>Price (₹)<input type="number" min="1" required value={form.price || ''} onChange={event => setForm({ ...form, price: Number(event.target.value) })}/></label><label>Fuel type<select value={form.fuelType} onChange={event => setForm({ ...form, fuelType: event.target.value })}>{['Petrol', 'Diesel', 'Electric', 'Hybrid', 'CNG'].map(value => <option key={value}>{value}</option>)}</select></label><label>Transmission<select value={form.transmission} onChange={event => setForm({ ...form, transmission: event.target.value })}><option>Manual</option><option>Automatic</option></select></label><label>Kilometres driven<input type="number" min="0" required value={form.kmDriven} onChange={event => setForm({ ...form, kmDriven: Number(event.target.value) })}/></label><label>Location<input required value={form.location} onChange={event => setForm({ ...form, location: event.target.value })} placeholder="e.g. Chennai"/></label><label>Status<select value={form.status} onChange={event => setForm({ ...form, status: event.target.value as Car['status'] })}><option value="active">Active</option>{form.status === 'sold' && <option value="sold">Sold</option>}<option value="hidden">Hidden</option></select></label></div></section><section className="admin-panel"><h2>Description &amp; features</h2><label>Description<textarea rows={5} value={form.description} onChange={event => setForm({ ...form, description: event.target.value })} placeholder="Describe the car, condition and highlights"/></label><FeaturePicker value={form.features || []} onChange={features => setForm(current => ({ ...current, features }))}/></section>
        <section className="admin-panel"><h2>Posted by</h2><p className="muted">Filled in from your account. Change it if you are listing on someone else&apos;s behalf. Shown to buyers and in the admin table.</p><div className="admin-form-grid"><label>Name<input value={form.postedBy?.name || ''} onChange={event => setForm({ ...form, postedBy: { ...(form.postedBy || {}), name: event.target.value } })} placeholder="Your name"/></label><label>Email<input type="email" value={form.postedBy?.email || ''} onChange={event => setForm({ ...form, postedBy: { ...(form.postedBy || {}), email: event.target.value } })} placeholder="you@example.com"/></label><label>Phone<input type="tel" value={form.postedBy?.phone || ''} onChange={event => setForm({ ...form, postedBy: { ...(form.postedBy || {}), phone: event.target.value } })} placeholder="Contact number"/></label></div></section><section className="admin-panel"><h2>Photos ({form.images.length}/7)</h2><p className="muted">Add 1 to 7 JPEG, PNG or WebP photos (up to 5 MB each). Select several files at once. Stored in {signedInAs?.storage === 'drive' ? 'Google Drive' : 'Cloudinary'}.</p><div className="image-preview-grid">{form.images.map((image, index) => <div key={`${image.url}-${index}`} className="image-preview"><img src={image.url} alt={`Car photo ${index + 1}`}/><button type="button" aria-label="Remove image" onClick={() => setForm({ ...form, images: form.images.filter((_, current) => current !== index) })}><X size={16}/></button></div>)}{form.images.length < 7 && <label className="upload-tile"><Upload size={22}/><span>{busy ? 'Uploading...' : 'Upload image'}</span><input type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={event => { uploadImages(event.target.files); event.target.value = ''; }}/></label>}</div></section><div className="form-actions"><button type="button" className="button button-secondary" onClick={() => setEditing(null)}>Cancel</button><button className="button button-primary" disabled={busy || form.images.length < 1}><Save size={18}/>{busy ? 'Saving...' : 'Save car'}</button></div></form></>}
      ...
      {tab === 'reports' && !editing && <><div className="admin-page-head"><div><span className="admin-kicker">ANALYTICS &amp; REPORTS</span><h1>Sales &amp; enquiries<span className="accent-dot">.</span></h1><p>How many cars sold, by whom, to which customers — and what buyers are asking about.</p></div>{report && <button type="button" className="button button-secondary" disabled={reportBusy} onClick={() => loadReport(token)}><RotateCcw size={16}/> Refresh</button>}</div>
        ...
        {!report ? <div className="admin-panel"><p className="panel-empty">{reportBusy ? 'Building the report…' : 'No report data yet.'}</p></div> : <>
          ...
          <div className="report-section-head"><div><span className="admin-kicker">ENQUIRIES</span><h2>What buyers are asking about<span className="accent-dot">.</span></h2></div>{report.range.salesperson && <span className="report-note"><AlertTriangle size={14}/> Enquiries are not tied to a salesperson, so this section covers everyone.</span>}</div>

          <div className="admin-stats report-stats">
            <div><span>Enquiries</span><strong>{report.enquiries.selected.total}</strong><Inbox/></div>
            <div><span>New</span><strong>{report.enquiries.selected.new}</strong><MessageSquareText/></div>
            <div><span>Contacted</span><strong>{report.enquiries.selected.contacted}</strong><Phone/></div>
            <div><span>Closed</span><strong>{report.enquiries.selected.closed}</strong><Check/></div>
          </div>

          <div className="report-quick">{([['This week', report.enquiries.quick.week], ['This month', report.enquiries.quick.month], ['This year', report.enquiries.quick.year], ['All time', report.enquiries.quick.allTime]] as [string, number][]).map(([label, value]) => <div key={label} className="report-quick-card"><small>{label}</small><strong>{value} {value === 1 ? 'enquiry' : 'enquiries'}</strong><span>&nbsp;</span></div>)}</div>

          <div className="admin-panel"><div className="panel-title"><h2>Enquiries through {report.range.chartYear}</h2><span className="report-chart-total">{report.enquiries.monthly.reduce((sum, m) => sum + m.count, 0)} enquiries</span></div>
            <div className="report-chart" role="img" aria-label={`Enquiries received each month of ${report.range.chartYear}`}>{report.enquiries.monthly.map(entry => { const peak = Math.max(...report.enquiries.monthly.map(m => m.count), 1); return <div key={entry.month} className="report-bar" title={`${entry.label}: ${entry.count} ${entry.count === 1 ? 'enquiry' : 'enquiries'}`}><span className="report-bar-value">{entry.count || ''}</span><span className="report-bar-fill" style={{ height: `${entry.count ? Math.max(6, (entry.count / peak) * 100) : 2}%` }}/><small>{entry.label}</small></div>; })}</div>
          </div>

          <div className="admin-panel"><div className="panel-title"><h2>Most asked about</h2><span>{report.enquiries.topCars.length} {report.enquiries.topCars.length === 1 ? 'car' : 'cars'}</span></div>
            {report.enquiries.topCars.length ? <div className="table-scroll"><table className="admin-report-table"><thead><tr><th>CAR</th><th>ENQUIRIES</th><th>LAST ASKED</th><th>ACTIONS</th></tr></thead><tbody>{report.enquiries.topCars.map(entry => <tr key={entry.carId}>
              <td><div className="table-user"><span className="user-avatar"><CarFront size={15}/></span><div><strong>{entry.car}</strong></div></div></td>
              <td><strong>{entry.count}</strong></td>
              <td>{formatDate(entry.lastAt)}</td>
              <td><div className="row-actions">{entry.slug && <a href={`/cars/${entry.slug}`} target="_blank" rel="noopener noreferrer">View listing</a>}</div></td>
            </tr>)}</tbody></table></div> : <p className="panel-empty">No enquiries in this period.</p>}
          </div>

          <div className="admin-panel"><div className="panel-title"><h2>Enquiry log</h2><span>{report.enquiries.recent.length} {report.enquiries.recent.length === 1 ? 'enquiry' : 'enquiries'}</span></div>
            {report.enquiries.recent.length ? <div className="table-scroll"><table className="admin-report-table"><thead><tr><th>CUSTOMER</th><th>CAR</th><th>CITY</th><th>RECEIVED</th><th>STATUS</th></tr></thead><tbody>{report.enquiries.recent.map(entry => <tr key={entry._id}>
              <td><div className="customer-cell"><strong>{entry.name}</strong>{entry.phone && <a href={`tel:${entry.phone}`}>{entry.phone}</a>}{entry.email && <a href={`mailto:${entry.email}`}>{entry.email}</a>}</div></td>
              <td>{entry.car}</td>
              <td>{entry.city || '—'}</td>
